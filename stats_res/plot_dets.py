...
def imshow_gt_det_bboxes(img,
    ...
    gt_colors = []
    for gts in eval_results:
        gt_covered = gts['gt_covered']
        gt_c = []
        for cov in gt_covered:
            c = 'white' if cov else 'yellow'
...
# mean_ap, eval_results = eval_map([result], [annotation])

# result[0] = result[0][np.where(eval_results[0]['tp'])[1]]
# mean_ap, eval_results = eval_map([result], [annotation])
#
# img = os.path.join(dataset.img_prefix, dataset.coco.imgs[0]['filename'])
# imshow_gt_det_bboxes(img, annotation, result, ['V', 'P', 'C'], fig_size=(45, 40), thickness=0, font_size=8,

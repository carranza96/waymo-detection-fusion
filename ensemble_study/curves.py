...
labels = ["Fuse", "FRCNN", "RetinaNet", "WBF", "NMS"]
ths = np.arange(0.05, 1, 0.01)
aps = [[] for _ in range(len(models))]
tps = [[] for _ in range(len(models))]
fps = [[] for _ in range(len(models))]
for sc in ths:

...
def eval_map(det_results,
    ...
    for i in range(1):
        ...
        #         in zip(cls_dets, cls_gts, cls_gts_ignore, [iou_thr for _ in range(num_imgs)], [area_ranges for _ in range(num_imgs)])]
...
def print_map_summary(mean_ap,
    ...
    for i in range(num_scales):
        ...
        for j in range(num_classes):
            tps = int(sum([np.sum(tps) for tps in results[j]['tp']]))
            fps = int(sum([np.sum(fps) for fps in results[j]['fp']]))
